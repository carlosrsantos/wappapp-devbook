$('#deixar-de-seguir').on('click', pararDeSeguir);
...
function pararDeSeguir() {
  ...
  $.ajax({
    url: `/usuarios/${usuarioId}/deixar-de-seguir`,
    method: 'POST',
  }).done(function(){
    windows.location = `/usuarios/${usuarioId}`;   
  }).fail(function(){
    Swal.fire("Ops...", "Erro ao deixar de seguir o usuário!", "error");
    $('#deixar-de-seguir').prop('disabled', false);
  });
}
function seguir() {
  const usuarioId = $(this).data('usuario-id');
